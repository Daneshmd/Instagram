...
def register_user():
    ...
    while True:
        #valid boodan email
        email= Prompt.ask("Email (or type back to cancel)")
        if email.lower() == 'back':
            return None
    ...
    save_data()
    console.print(f"[green]{username}'s account has been created successfully![/]")
    return username
